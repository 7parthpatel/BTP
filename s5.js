...
function datagenerate()
{
	...
	totalvehicle = []
		...
		TMP = [5,a,b,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV];
		...
		five = setInterval(function(){	rn = Math.floor(Math.random() * ( latitude[4].length - 0 )) + 0; lat = latitude[4][rn];	lng = longitude[4][rn];/*allv = [5,lat,lng];client.publish('sensor1',allv.toString());*/console.log(5,lat,lng);},(60000/totalvehicle[0][13]));

		setTimeout(function(){clearInterval(five);client.publish('sensor1',TMP.toString());
