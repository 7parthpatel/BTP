...
function datagenerate()
{
	...
	totalvehicle = []
		...
		TMP = [4,a,b,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV];
		...
		four = setInterval(function(){	rn = Math.floor(Math.random() * ( latitude[3].length - 0 )) + 0; lat = latitude[3][rn];	lng = longitude[3][rn];/*allv = [4,lat,lng];client.publish('sensor1',allv.toString());*/	console.log(4,lat,lng);},(60000/totalvehicle[0][13]));
	
		setTimeout(function(){clearInterval(four);
			console.log("over");console.log(totalvehicle);client.publish('sensor1',TMP.toString());/*var sql = "INSERT INTO trafficdata (sensorId,Date,Time,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV) VALUES ?";
		connection.query(sql, [totalvehicle], function(err) {
		});*/	},60000);
