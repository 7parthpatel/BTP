...
function datagenerate()
{
	...
	totalvehicle = []
		...
		TMP = [7,a,b,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV];
		...
		seven = setInterval(function(){	rn = Math.floor(Math.random() * ( latitude[6].length - 0 )) + 0; lat = latitude[6][rn];	lng = longitude[6][rn];/*allv = [7,lat,lng];client.publish('sensor1',allv.toString());*/	console.log(7,lat,lng);},(60000/totalvehicle[0][13]));
	
		setTimeout(function(){clearInterval(seven);client.publish('sensor1',TMP.toString());
			console.log("over");console.log(totalvehicle);
			/*var sql = "INSERT INTO trafficdata (sensorId,Date,Time,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV) VALUES ?";
		connection.query(sql, [totalvehicle], function(err) {
		});*/	},60000);
