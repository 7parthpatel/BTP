...
function datagenerate()
{
	...
	totalvehicle = []
		...
		TMP = [6,a,b,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV];
		...
		six = setInterval(function(){	rn = Math.floor(Math.random() * ( latitude[5].length - 0 )) + 0; lat = latitude[5][rn];	lng = longitude[5][rn];/*allv = [6,lat,lng];client.publish('sensor1',allv.toString());*/	console.log(6,lat,lng);},(60000/totalvehicle[0][13]));

		setTimeout(function(){clearInterval(six);client.publish('sensor1',TMP.toString());
