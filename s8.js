...
function datagenerate()
{
	...
	totalvehicle = []
		...
		TMP = [8,a,b,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV];
		...
		i=7
		rn = Math.floor(Math.random() * ( latitude[i].length - 0 )) + 0; lat = latitude[i][rn];lng = longitude[i][rn];/*allv = [i+1,lat,lng];client.publish('sensor1',allv.toString());*/console.log(i+1,lat,lng);//}

		
		eight = setInterval(function(){	rn = Math.floor(Math.random() * ( latitude[7].length - 0 )) + 0; lat = latitude[7][rn];	lng = longitude[7][rn];/*allv = [8,lat,lng];client.publish('sensor1',allv.toString());*/	console.log(8,lat,lng);},(60000/totalvehicle[0][13]));
	
		setTimeout(function(){clearInterval(eight);client.publish('sensor1',TMP.toString());
			console.log("over");console.log(totalvehicle);/*var sql = "INSERT INTO trafficdata (sensorId,Date,Time,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV) VALUES ?";
		connection.query(sql, [totalvehicle], function(err) {
		});*/	},60000);
		
		//client.publish('tempMeasurement',TMP.toString())
