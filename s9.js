...
function datagenerate()
{
	...
	totalvehicle = []
		...
		TMP = [9,a,b,WMV2,CarV,BusV,LGV,HGVR2,HGVR3,HGVR4,HGVA3,HGVA5,HGV,AMV];
		...
		i=8
		rn = Math.floor(Math.random() * ( latitude[i].length - 0 )) + 0; lat = latitude[i][rn];lng = longitude[i][rn];/*allv = [9,lat,lng];client.publish('sensor1',allv.toString());*/console.log(i+1,lat,lng);//}

		
		nine = setInterval(function(){	rn = Math.floor(Math.random() * ( latitude[8].length - 0 )) + 0; lat = latitude[8][rn];	lng = longitude[8][rn];/*allv = [9,lat,lng];client.publish('sensor1',allv.toString());*/	console.log(9,lat,lng);},(60000/totalvehicle[0][13]));
		
		setTimeout(function(){clearInterval(nine);client.publish('sensor1',TMP.toString());
